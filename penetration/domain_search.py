...
import sys  
import re  
from domain_search_conf import *  
...
def bing_search(site, page):  
    ...
    for p in range(int(page)):  
        try:  
            ...
            # url = "https://www.bing.com/search?q=site:{0}&qs=n&sp=-1&pq=site:{0}" \  
            #       "&sc=0-18&sk=&cvid=8D775C5402784E6096D8B5C3A3BA386A&first={1}&FORM=PERE".format(site, p)            html = requests.get(url, headers=headers, timeout=3).content.decode()  
        except Exception:  
            pass  
        job_bt = re.findall('<a target="_blank" target="_blank" href="(.*?)"', html)  
        for h in job_bt:  
